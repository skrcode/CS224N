...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope('classifier') as scope:
            W = tf.Variable(np.zeros([self.config.n_features,self.config.n_classes]),name = 'W')
            b = tf.Variable(np.zeros([self.config.batch_size,self.config.n_classes]),name = 'b')
            
            #W = tf.reshape(W, [self.config.n_features,self.config.n_classes])
            #b = tf.reshape(b, [self.config.batch_size,self.config.n_classes])
            W = tf.to_float(W)
            b = tf.to_float(b)
